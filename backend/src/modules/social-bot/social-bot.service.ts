...
export class SocialBotService {
  ...
  async joinTournament(userId: string, tournamentId: string) {
    ...
    await this.repo.addTournamentParticipant({
      tournamentId: ObjectId.isValid(tournamentId) ? new ObjectId(tournamentId) : tournamentId,
      userId,
      joinedAt: new Date(),
      status: 'active',
    })
  ...
  async withdrawTournament(userId: string, tournamentId: string) {
    const normalizedId = ObjectId.isValid(tournamentId) ? new ObjectId(tournamentId) : tournamentId
    const result = await this.repo.removeTournamentParticipant(normalizedId, userId)
    if (result.deletedCount === 0) {
      throw new BadRequestException('User is not a participant of the tournament')
    }
  ...
  async moveBot(userId: string, dto: BotMoveDto) {
    ...
    const requestLog = await this.repo.createBotMoveRequest({
      sessionId: ObjectId.isValid(dto.sessionId) ? new ObjectId(dto.sessionId) : dto.sessionId,
      requestPayload: {
        userId,
        fen: dto.fen,
      },
    ...
    await this.repo.updateBotMoveRequestResponse(requestLog._id as ObjectId, move, 'done')
