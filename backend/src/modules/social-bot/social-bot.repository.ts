...
export class SocialBotRepository {
  ...
  async removeTournamentParticipant(tournamentId: ObjectId | string, userId: string) {
    return this.db.collection('tournament_participants').deleteOne({ tournamentId, userId })
  }
  ...
  async updateBotMoveRequestResponse(id: ObjectId, responsePayload: unknown, status: string) {
    await this.db
      .collection('bot_move_requests')
      .updateOne({ _id: id }, { $set: { responsePayload, status, updatedAt: new Date() } })
  }
